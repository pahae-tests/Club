...
export default function AdminActualites({ isDark }) {
  ...
  const [actualites, setActualites] = useState([
    {
      id: 1,
      titre: "Nouvelle faille critique dans OpenSSL",
      descr: "Discussion sur la faille CVE-2025-1234 et ses implications pour les serveurs web modernes.",
      date: "2025-10-20",
      heure: "15:00",
      salle: "Amphi B, FST Settat"
    },
    {
      id: 2,
      titre: "Atelier : Phishing et Ingénierie Sociale",
      descr: "Présentation des techniques modernes de phishing et comment s'en protéger.",
      date: "2025-10-10",
      heure: "14:30",
      salle: "Salle 104, FST Settat"
    }
  ]);
  ...
  const handleEditActualite = (actualite) => {
    ...
    setEditingActualite(actualite.id);
    setShowAddModal(true);
  };
  ...
  const handleSaveActualite = () => {
    ...
    if (editingActualite) {
      setActualites(actualites.map(a => a.id === editingActualite ? { ...formData, id: editingActualite } : a));
    } else {
      setActualites([...actualites, { ...formData, id: Date.now() }]);
    }
          ...
          <div className={`rounded-3xl overflow-hidden backdrop-blur-xl ${
            ...
            <div className="divide-y divide-white/5">
              {actualites.map((actualite, index) => (
                <div key={actualite.id} className="p-6" style={{ animation: `fadeIn 0.5s ease-out ${index * 0.1}s both` }}>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => setExpandedActualite(expandedActualite === actualite.id ? null : actualite.id)}
                        className={`p-2 rounded-lg ${isDark ? 'bg-gray-700/50 hover:bg-gray-700' : 'bg-gray-200 hover:bg-gray-300'} transition-all`}
                      >
                        {expandedActualite === actualite.id ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                      </button>
                      <button
